...
/**
 * Configure Gradle JVM heap size and architecture filters in gradle.properties.
 *
 * This ensures Android builds have enough memory for the D8 dexer and
 * limits native compilation to the specified ABIs (avoiding unnecessary
 * cross-compilation for emulator-only architectures).
 *
 * Must be called after prebuild (when android/ directory exists).
 */
export async function configureGradleProperties(
  projectDir: string,
  ...
  let content = '';
  try {
    content = await fs.promises.readFile(gradlePropsPath, 'utf-8');
  } catch {
  ...
  // JVM heap — 4GB is safe on GitHub Actions runners (16GB RAM)
  // and prevents OOM during mergeExtDexRelease for large apps.
  // Replace Expo's default 2GB if present, otherwise add.
  const jvmArgsLine = 'org.gradle.jvmargs=-Xmx4g -XX:MaxMetaspaceSize=1g';
  if (content.match(/^org\.gradle\.jvmargs\s*=/m)) {
    content = content.replace(/^org\.gradle\.jvmargs\s*=.*$/m, jvmArgsLine);
    core.info('Gradle JVM heap overridden: -Xmx4g -XX:MaxMetaspaceSize=1g');
    modified = true;
  } else {
    content += (content.endsWith('\n') ? '' : '\n') + jvmArgsLine + '\n';
    core.info('Gradle JVM heap configured: -Xmx4g -XX:MaxMetaspaceSize=1g');
    modified = true;
  }

  // Architecture filtering — limits CMake native compilation to specified ABIs
  if (architectures && architectures !== 'all') {
    const archLine = `reactNativeArchitectures=${architectures}`;
    if (content.match(/^reactNativeArchitectures\s*=/m)) {
      content = content.replace(/^reactNativeArchitectures\s*=.*$/m, archLine);
    } else {
      content += (content.endsWith('\n') ? '' : '\n') + archLine + '\n';
    }
    core.info(`Android architectures configured: ${architectures}`);
    modified = true;
  }

  if (modified) {
    await fs.promises.writeFile(gradlePropsPath, content, 'utf-8');
  }
}
