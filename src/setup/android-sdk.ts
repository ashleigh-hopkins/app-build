...
export async function configureGradleProperties(
  ...
  if (!content.includes('org.gradle.jvmargs')) {
    additions.push('org.gradle.jvmargs=-Xmx4g -XX:MaxMetaspaceSize=1g');
  ...
  if (architectures && architectures !== 'all' && !content.includes('reactNativeArchitectures')) {
    additions.push(`reactNativeArchitectures=${architectures}`);
    core.info(`Android architectures configured: ${architectures}`);
  }

  if (additions.length > 0) {
    const separator = content.length > 0 && !content.endsWith('\n') ? '\n' : '';
    const header = '\n# Generated by app-build action\n';
    const newContent = content + separator + header + additions.join('\n') + '\n';
